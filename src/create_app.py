...
def make_predictions(
    config: DictConfig, dummy_df: pd.DataFrame, model: XGBClassifier
):
    """Transform the data then make predictions"""

    feature_str = " + ".join(config.process.features)
...
def write_predictions(config: DictConfig, data: dict, model: XGBClassifier):
    if st.button("Will this employee leave in 2 years?"):
        df = pd.DataFrame(data, index=[0])
        dummy_df = add_dummy_data(df)
        prediction = make_predictions(config, dummy_df, model)
...
def main(config: DictConfig):
    ...
    write_predictions(config, data, model)
